...
//    private $_file_path;
    ...
    public function __construct($sql, $sql_key=array(), $sql_table='', $db_connection= array()){
		...
		$this->jq_rowList   = array(50, 100, 250, 500);
		...
		$loading_text = '';
		if(strpos($_SERVER['REQUEST_URI'],'contributor_listing.php')){  $loading_text = 'Loading Contributors'; }
		else if(strpos($_SERVER['REQUEST_URI'],'school_contribution_profile.php')){ $loading_text = 'Loading Contributor&#39s Profile'; }
		else if(strpos($_SERVER['REQUEST_URI'],'alef-all-schools.php')){ $loading_text = 'Loading All Schools'; }
        $this->jq_loadtext  = $loading_text;
        ...
        $this->_ver_num             = 'phpGrid(v6.1) {jqGrid:v4.5.2, jQuery:v1.9.0, jQuery UI:1.10.0}';
        $this->sys_msg              = null;
        $this->form_width			= 450;
        $this->form_height			= '100%';		// invalide height, jqgrid will resize the height to best fit the dialog.
        $this->alt_colors           = array('hover'=>'#F2FC9C', 'highlight'=>'yellow', 'altrow'=>'#F5FAFF');
    ...
    public function set_colModel($results){
        ...
        for($i=0;$i<$this->_num_fields;$i++){
            ...
            /*
            if(isset($this->col_edittypes[$col_name]) && ($this->col_edittypes[$col_name]['type']=='custom')){
                $editoptions['custom_element'] = '###function(value, option){'.
                                                    'var ac=$(\'<input type=text>\');'.
                                                    'ac.val(value);'.
                                                    'ac.autocomplete({source:availableTags});'.
                                                    'return ac;'.
                                                '}###';
                $editoptions['custom_value'] = '###function(el, op, value){'.
                                                    'if(op == \'set\'){ $(el).val(value);}'.
                                                    'return $(el).val();'.
                                                '}###';
            ...
            */
            /*
            if(isset($this->col_edittypes[$col_name]) && ($this->col_edittypes[$col_name]['type']=='select')){
                $editoptions['dataInit'] = '###function(el){$(el).autocomplete({source:availableTags})}###';
            }
            */
                        ...
                        oneditfunc:function(){' ."\n";
                            if(!empty($this->col_autocomplete)){
                                foreach($this->col_autocomplete as $col_name){
                                    echo  '$("#'. $this->jq_gridName .' tr#"+id+" td select[id="+id+"_'. $col_name .']").select2({width:"100%",minimumInputLength:2});';
                                } // foreach
                            }
                            // INLINE edit WYSIWYG
            ...
            case 'FORM':
                ...
                if((strrpos($this->edit_options,"U")!==false)){
                    ...
                    if(!empty($this->col_autocomplete)){
                        foreach($this->col_autocomplete as $col_name){
                            $editEvtHanlder .=
                                '$("#FrmGrid_'. $this->jq_gridName .' select[id='. $col_name .']").select2({minimumInputLength:2});';
                        } // foreach
                    }
                    ...
                    if(!empty($this->col_wysiwyg)){
                        ...
                        foreach($this->col_wysiwyg as $key => $value){
                            $editEvtHanlder  .=
                               ...
                               /* for future consideration. jwysiwyg is sufficient for now
                               '$("#'.$key.'").addClass("tinymce_wysiwyg");
                                tinymce.init({
                                    selector: ".tinymce_wysiwyg",
                                    plugins: [
                        ...
                        $editEvtHanlder  .='},
                            "onclickSubmit" : function( params, posdata ) {return true; },
                            "onClose" : function (){';
                                foreach($this->col_wysiwyg as $key => $value){
    ...
    private function display_subgrid(&$cnt){
        if($this->obj_subgrid != null){
            ...
            $this->obj_subgrid->set_jq_url($this->obj_subgrid->get_jq_url().'+row_id'.$cnt, false);
            $this->obj_subgrid->set_jq_pagerName('pager_id'.$cnt, false);
        ...
        if($this->export_type!=null){
            echo 'jQuery("#'. $this->jq_gridName .'").jqGrid("navButtonAdd",'. $this->jq_pagerName .',{caption:"",title:"'. $this->export_type .'",
                        onClickButton:function(e){
                            try{
                                jQuery("#'. $this->jq_gridName .'").jqGrid("excelExport",{url:"'. $this->export_url .(($this->export_type!='')?'&export_type='. $this->export_type:'') .'"});
                            } catch (e) {
