...
if($searchOn) {
    ...
    if(isset($_REQUEST['filters']) && $_REQUEST['filters'] !=''){
        ...
        for($i=0;$i<count($rules);$i++){ 
			if( strpos($rules[$i]->field, "school_") === false )
			{   
			...
			else
			{
				if($sqlHaving != "")
					$sqlHaving .=" AND ";
				$sqlHaving .=  " replace( replace( replace(".$rules[$i]->field.",'$',''),',','' ),'.00','' ) ".sprintf($op[$rules[$i]->op],$rules[$i]->data); // = '".$rules[$i]->data."'";
                	//sprintf($op[$rules[$i]->op],$rules[$i]->data);   
			}	
        }
    }
...
if($sidx)
$sidx = " CAST( replace( replace( $sidx  ,'$',''),',','' ) AS DECIMAL ) ";
...
if($sql_filter != '' && $searchOn && $sqlWhere !=""){
	$SQL = $grid_sql .' WHERE '. $sql_filter .' AND '. $sqlWhere;
}elseif($sql_filter != '' && !$searchOn  ){
	$SQL = $grid_sql .' WHERE '. $sql_filter;
}elseif($sql_filter == '' && $searchOn && $sqlWhere !="" ){
	$SQL = $grid_sql .' WHERE '. $sqlWhere;
